...
func GetTimePeriod(start, end time.Time) string {
	if end.Before(start) {
		start, end = end, start
	}
	diff := end.Sub(start)
	...
	// If range spans 3 or more calendar months (inclusive) → monthly buckets
	// Example: Jan 1 → Mar 31 has months=2 but spans 3 calendar months.
	if months+1 >= 3 {
		return "month"
	}
