...
export default function OsVisitorDialog({
  ...
  const osVisitorsMoreData = osVisitorsMore?.data ?? [];
  ...
  if (isLoading) {
    ...
    dialogBody = (
      <div className="h-[60vh] w-full flex items-center justify-center">
        <p className="text-red-500 text-center">Failed to load os visitors</p>
      </div>
    );
  } else if (osVisitorsMoreData.length === 0) {
    dialogBody = (
      <div className="h-[60vh] w-full flex items-center justify-center">
        <p className="text-muted-foreground text-center">No os visitors data</p>
      </div>
    );
  } else {
    dialogBody = (
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-2xl rounded">
        <DialogHeader>
          <DialogTitle className="text-xl border-b pb-4">OS</DialogTitle>
        </DialogHeader>
        {dialogBody}
      </DialogContent>
    </Dialog>
